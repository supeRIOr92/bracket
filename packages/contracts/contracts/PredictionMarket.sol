    ...
    ReentrancyGuardUpgradeable
{
     ...
     * @param _usdc            USDC token address (Base Mainnet: 0x833589...)
     * @param _priceFeed       Chainlink BTC/USD Feed (Base Mainnet)
     * @param _devTreasury     Safe Multisig untuk developer fee
     * @param _jackpotTreasury Safe Multisig untuk jackpot
     * @param _flywheelTreasury Safe Multisig untuk flywheel
     * @param _owner           Admin address (Safe Multisig)
     */
    function initialize(
        address _usdc,
        address _priceFeed,
     ...
     * @dev Digunakan jika ada rotasi Safe Multisig.
     */
    ...
    function placeBet(
        ...
        // Max bet check (berlaku jika total pool >= 10k USDC)
        ...
        // Transfer USDC dari user ke contract
     ...
     *
     * MARKET VALIDITY RULE:
     *      Settlement normal jika:
     *      (1) Minimal 2 pool berbeda terisi (poolsWithStake >= 2)
     *      (2) Winning pool bukan satu-satunya pool yang terisi
     *      Jika tidak → refund semua peserta, zero fee.
     *
    ...
    function settleMarket(uint256 marketId) external nonReentrant {
        ...
        if (updatedAt < m.closeAt) revert OraclePriceTooStale();
        ...
        if (!isRefund) {
            // ── Normal settlement: distribute fees ───────────────────────────
        ...
        // Jika refund: semua dana tetap di contract, diklaim per-user via claim()
     ...
     * @dev - Normal settlement: hanya pemenang (bet di winning pool) yang bisa claim.
     *      - Refund mode: semua peserta bisa claim kembali stake penuh mereka.
    ...
    function claim(uint256 marketId) external whenNotPaused nonReentrant {
        ...
        if (m.refunded) {
            // ── Refund mode: kembalikan stake penuh ──────────────────────────
            payout = userBet.amount;
        } else {
            // ── Normal mode: hanya pemenang ──────────────────────────────────
    ...
    /**
     * @notice Estimasi payout untuk taruhan tertentu (berubah seiring pool berubah).
     * @param marketId  ID market
    ...
    /**
     * @dev Menentukan pool pemenang berdasarkan harga Chainlink.
    ...
    /**
     * @dev Cek apakah market harus direfund.
     *
     *      Kondisi REFUND (salah satu terpenuhi):
     *      (A) Kurang dari 2 pool yang terisi (poolsWithStake < 2)
     *      (B) Winning pool adalah satu-satunya pool yang terisi
     *
     *      Kondisi NORMAL (keduanya terpenuhi):
     *      (A) Minimal 2 pool berbeda terisi
     *      (B) Winning pool bukan satu-satunya pool yang terisi
     */
    function _checkRefundCondition(
        ...
        // Kondisi A: kurang dari 2 pool terisi
        if (poolsWithStake < 2) return true;

        // Kondisi B: winning pool = satu-satunya pool terisi
        // (edge case: mungkin ada >=2 pool terisi tapi sisanya bukan winning)
        // Sebenarnya jika poolsWithStake >= 2, kondisi B sudah otomatis false
        // karena ada pool lain yang terisi. Tapi kita explicit untuk clarity:
    ...
    /**
     * @dev Ambil market dari storage, revert jika tidak ada.
     */
    ...
    /**
     * @dev Hanya owner (Safe Multisig) yang bisa upgrade.
     */
