...
export class AuthService {
  ...
  /**
   * Verifikasi Privy access token dan return JWT kita sendiri.
   * Flow:
   * 1. Kirim token ke Privy API untuk verifikasi
   * 2. Extract wallet address dari response
   * 3. Upsert user ke Supabase
   * 4. Return JWT
   */
  async login(privyAccessToken: string) {
    // Verifikasi token ke Privy
  ...
  /**
   * Verifikasi Privy token via Privy REST API.
   */
   ...
   * Kalau belum ada → create + buat user_stats default.
  ...
  /**
   * Validate user dari JWT payload (dipanggil oleh JwtStrategy).
   */
